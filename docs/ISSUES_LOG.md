...
### e2e-02「應能取得目前登入教師姓名」在整組跑時偶發失敗 ✅ 已修（2026-09-20）
...
修法：新增 `helpers.mjs` 的 `waitForIdentityName()`，輪詢等到 `#user-name` 內出現 `.v2-role-tag` 且首個 text node 是單一姓名才回傳，取代固定等待。2026-09-20 整組實跑該案通過。

---

## 2026-09-20 查核發現

### e2e-02「推薦清單標示同領域」的斷言已過時 🟡 測試層問題，產品行為正確

`npm run test:e2e` 整組實跑 27/28，失敗案為「代課教師推薦清單排除該時段有課的教師，並標示同領域」，訊息為「應有同領域教師標示（實際標籤：該時段空堂 ×5）」。

**根因**：`3a39f80`（2026-09-18，推薦改五層順序）把 `recommendationEngine.js` 的標籤文字從「同領域教師（X）」改成「同科目（X）」「該班導師（X）」「同任課班級（X）」，但沒有同步更新 `e2e-02` 的 `recommendedTeachers()`——它的正則只認 `同領域教師（` 與 `該時段空堂`，於是有同科目標示的教師**整個沒被收進待驗清單**，剩下的自然全是「該時段空堂」，斷言必然失敗。

**是否為產品缺陷**：不是。`test/test-recommendation.mjs` 14 項全通過，推薦引擎確實有標示同科目與班導師。

**修法**：把 `e2e-02:40` 與 `:43` 的正則改為涵蓋新標籤（`同科目（|該班導師（|同任課班級（|同領域（|該時段空堂`），並把該案名稱與斷言改為驗證「同科目」而非「同領域」。未修。
...
以真實瀏覽器操作畫面測出，見 `test/e2e/README.md`。

### 一般教師的「原任課教師」下拉未鎖定為本人 ✅ 已修（2026-09-20）

- **描述**: 一般教師登入後，「調代課申請 → 步驟一 → 原任課教師」下拉仍可選到全校任何一位教師（實測 20 位），UI 沒有鎖定為本人，欄位也未 disabled。
- **是否有資料風險**: 沒有。`firestore.rules` 擋下代他人發起（情境 2 的「教師代他人發起申請被拒」實測回 403）。
- **影響**: 教師可能一路填完四個步驟才在送出時被拒，不知道原因。
- **修法**: `v2-app.js` 新增 `applyOwnTeacherLock()`，身份解析完成後把下拉收斂成只剩本人並 disabled；並以 `patchOwnTeacherLock()` 掛在 V1 `populateTeacherDropdowns` 後面，每次重填都重新套用。另補 `restoreOwnTeacherSelection()`——`resetSubstituteFlow()` 送出後會把欄位設為 `''`，鎖定後選單無空值選項會變成「沒有選中項」且欄位已停用，教師送完一張就不能再送第二張。admin 不受影響。

### 「教師管理」頁籤對教學組長也可見 ✅ 已修（2026-09-20）

- **描述**: 教學組長看到的頁籤與教務主任完全相同，包含「教師管理」。
- **是否有資料風險**: 沒有。`teachers` 的 create/update/delete 在規則層限定 director（情境 2/4 已驗證）。
- **修法**: 頁籤 class 由 `.v2-approver-only` 改為 `.v2-director-only`（機制原已存在，見 `v2-app.js:51`），與 `V2_PERMISSION_SYSTEM.md` 角色表及 `firestore.rules` 一致。e2e-01 的對應 `knownGap` 已改為正式斷言。

### 登入在 Firebase 初始化完成前會直接失敗 ✅ 已修（2026-08-11）

- **描述**: Firebase SDK 是啟動時才從 CDN 動態載入，初始化為非同步。`authService` 的六個對外操作（Google 登入／Email 登入／註冊／密碼重置／驗證信／建教師帳號）原本都是「沒初始化就丟 `請先完成 Firebase 設定`」——對使用者毫無意義的訊息，且**登入請求根本不會送出**（實測 Auth Emulator 完全沒收到請求），畫面只停在登入視窗。
- **重現**: 以瀏覽器在頁面載入後立刻登入，連續 6 次、6 次都是這個錯誤。網路慢時真實使用者也會踩到。
- **修法**: 統一改為先 `await ensureFirebaseReady()`（`initializeFirebase()` 本身有 in-flight promise 保護，重複呼叫共用同一次初始化）；真的失敗才提示「系統尚未完成啟動，請稍候再試一次」。

### e2e 環境下登入很慢 🟢 已查明，屬測試環境特性

- **描述**: 一度判定為「登入卡住、成功率只有兩成」，實際是**本機 emulator 的 Firestore 查詢極慢**：單一文件讀取 20～70ms，但集合查詢要數秒（實測 6.2s）；bootstrap 串行多個集合查詢，整段要 8～75 秒，課表由訂閱送達要 41～61 秒。
- **先前結論的錯誤**: e2e 只等 9～22 秒就判定「卡住」——它不是卡死，只是還在跑。把等待上限提高到 100 秒後，登入成功率由 2/6 變成 4/4。
- **是否影響正式環境**: 正式站連的是 Google 的 Firestore，不是本機 Java emulator，這裡的數字不構成正式環境的證據；也不宣稱正式環境一定沒有類似情形（不會拿正式站做這種驗證）。
- **已排除**: 種子資料、projectId 命名空間、`initAuthService` 重複掛監聽、`initializeFirebase` 併發、long-polling、重啟 emulator、每次全新瀏覽器、快取 Firebase SDK。
