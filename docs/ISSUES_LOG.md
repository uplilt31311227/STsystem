...
- **描述**: headless Chromium ＋ 本機 Emulator 的組合下，登入成功率實測約兩到四成。失敗時 Firebase 認證已成功但 bootstrap 的某個 Firestore 一次性查詢永不回應（不逾時、不拋錯、無 console 錯誤），`unlockV2App()` 不執行，畫面停在登入遮罩。
- **已排除**: 種子資料、projectId 命名空間、`initAuthService` 重複掛監聽、`initializeFirebase` 併發、long-polling、重啟 emulator、每次全新瀏覽器、快取 Firebase SDK。
- **未知**: 正式環境是否同樣受影響。真實使用者的條件（一般瀏覽器、有 HTTP 快取、連正式 Firestore）與此差異大，不應直接套用這個成功率。
- **現況**: e2e 以重試繞過，讓操作測試能問到它要問的問題；穩定度本身由 `e2e-00-login-stability` 專門量測並如實回報。
