...
- **一般教師的「原任課教師」鎖定為本人**：原本下拉仍列得出全校教師（實測 20 位），教師可以一路填完四個步驟，才在按下送出時被拒且看不出原因（`ISSUES_LOG.md` 2026-08-11）。改為身份解析完成後收斂成只剩本人並停用。這**不是安全邊界**——送出閘門在 `interceptSubmitButton()`，越權寫入另有 `firestore.rules` 擋下（情境 2 實測回 403），這層只是讓 UI 不要先給出做不到的選項。admin 不受影響，仍可代任一教師發起。另補 `restoreOwnTeacherSelection()`：`resetSubstituteFlow()` 送出成功後會把 `#sub-teacher` 設為 `''`，但鎖定後選單沒有空值選項，select 會變成沒有選中項且欄位已停用，教師送完一張就不能再送第二張。
- **教師管理頁籤改為 director 限定**：[[V2_PERMISSION_SYSTEM]] 的角色表早已寫明「教師管理／學校設定僅 director 可用」，`firestore.rules` 也是 `isDirector` 才放行，但頁籤掛的是 `.v2-approver-only`——組長看得到、點得進去、改得動欄位，要等寫入時才被規則擋下。改掛 `.v2-director-only`（機制已存在，見 `v2-app.js:51`）。
- **CSS 相容別名移除**：Stage 0 為了「零視覺變動」留下的 13 個舊變數（`--primary-color`／`--text-muted`／`--background-color` 等）已無使用者，最後兩個殘留點（`index.html` 課程 modal、`v2-app.js` 加入學校分隔線的 inline style）改用新 token 後，整段別名區塊移除。全庫掃描零殘留（唯一提及是 [[PLAN]] 第 56 行的歷史說明文字）。至此 CSS token 收斂完成，只剩單一組命名。
