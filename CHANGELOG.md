...
## [2026-09-20] 一般教師鎖定原任課教師欄位、教師管理頁籤改 director 限定、CSS 相容別名收尾

三件獨立的小改動，分別是一個體驗修復、一個權限對齊、一個重構收尾。

- **一般教師的「原任課教師」鎖定為本人**：原本下拉仍列得出全校教師（實測 20 位），教師可以一路填完四個步驟，才在按下送出時被拒且看不出原因（`docs/ISSUES_LOG.md` 2026-08-11）。改為身份解析完成後收斂成只剩本人並停用。這**不是安全邊界**——送出閘門在 `interceptSubmitButton()`，越權寫入另有 `firestore.rules` 擋下（情境 2 實測回 403），這層只是讓 UI 不要先給出做不到的選項。admin 不受影響，仍可代任一教師發起。另補 `restoreOwnTeacherSelection()`：`resetSubstituteFlow()` 送出成功後會把 `#sub-teacher` 設為 `''`，但鎖定後選單沒有空值選項，select 會變成沒有選中項且欄位已停用，教師送完一張就不能再送第二張。
- **教師管理頁籤改為 director 限定**：`V2_PERMISSION_SYSTEM.md` 的角色表早已寫明「教師管理／學校設定僅 director 可用」，`firestore.rules` 也是 `isDirector` 才放行，但頁籤掛的是 `.v2-approver-only`——組長看得到、點得進去、改得動欄位，要等寫入時才被規則擋下。改掛 `.v2-director-only`（機制已存在，見 `v2-app.js:51`）。
- **CSS 相容別名移除**：Stage 0 為了「零視覺變動」留下的 13 個舊變數（`--primary-color`／`--text-muted`／`--background-color` 等）已無使用者，最後兩個殘留點（`index.html` 課程 modal、`v2-app.js` 加入學校分隔線的 inline style）改用新 token 後，整段別名區塊移除。全庫掃描零殘留（唯一提及是 `docs/PLAN.md:56` 的歷史說明文字）。至此 CSS token 收斂完成，只剩單一組命名。

**驗證**：`npm run check` 39/39；`npm test` 單元 7 支 + 情境 37/37 全通過；`npm run test:e2e` 以 emulator 實跑 **27/28**。

**e2e 的失敗換了一案，需留意**：上一版 27/28 失敗的是 e2e-02「教師身份取得」的偶發失敗（emulator 變慢時身份區來不及渲染就被讀取），本次已隨測試調整修掉——改用新的 `waitForIdentityName()` 輪詢等待取代固定 1200ms。本次失敗的是另一案「代課教師推薦清單排除該時段有課的教師，並標示同領域」，**屬既有問題、非本次改動造成**：`3a39f80`（推薦改五層順序）把標籤文字從「同領域教師（X）」改成「同科目（X）」「該班導師（X）」，但沒有同步更新 `e2e-02` 的 `recommendedTeachers()` 正則，導致有同科目標示的教師整個沒被收進待驗清單，斷言必然失敗。**推薦引擎本身行為正確**（`test-recommendation.mjs` 14 項通過），是 e2e 斷言過時，待後續修正。

---

## [2026-09-20] 多節代課的班級聯改為每班獨立一張

多節代課（「一次選多節（同一天）」）原本只輸出一張綜合班級聯，列出所有班級的節次。例如王老師週一 1、3、5 節分別在 7年1班、7年2班、8年1班，由李老師代課，三個班拿到的是同一張、看得到彼此的異動。

- 班級聯改為依班級拆開：每班一張，標題顯示「7年1班 班級聯」，只列該班的節次與課表格。
- 原任課教師聯、代（調）課教師聯、教學組聯不變，仍列出全部節次。
- 每頁左右各一聯，聯數為奇數時最後一頁右半留白；頁尾「此單一式N聯」依實際聯數顯示（上例為一式六聯、共 3 頁）。同一班的多節代課仍是一式四聯、2 頁，與原本相同。
- 單節代課與 V2 審核通過後產生的 PDF 本來就是一筆一張，不受影響。

**驗證**：新增 `test/test-pdf-class-sheets.mjs`（8 項，已加入 `npm test`）；以 Playwright 在瀏覽器實跑上述三班情境，下載的 PDF 轉圖檢查：共 3 頁，三張班級聯各只含自己班的一節，教學組聯含全部三節。

---

## [2026-09-18] 代課推薦改為五層順序，新增「兼課」「同任課班級」標記，並列出全部可代課教師

**推薦順序**（`recommendationEngine.js`）：同科目 → 班導師 → 同任課班級 → 空堂 → 兼課。

- **同科目**：課表上該教師也教這一科，或教師「任教領域」欄手動填了這一科名（例如「數學」）。取代原本的「同領域」優先；同領域（例如國文與英語同屬語文）不再算一層，只在同層內微調排序、仍寫在說明文字裡。
- **班導師**：沿用既有「導師班級」設定。
- **同任課班級**（新）：課表上也有教這個班的教師，由課表自動判斷，不需手動設定。
- **兼課**（新）：教師管理新增「兼課教師」勾選（V1 教師屬性表與 V2 教師管理合併表皆有），存於 `teacher.partTime`。兼課教師不論是否同科目／導師，一律排在最後一層。V2 表中尚未出現在課表的教師，兼課勾選為停用（兼課不會從 V2 副本回填，勾了也不會生效）；儲存時先寫 Firestore、成功才寫本機，失敗只需還原勾選。
- 同層內符合條件越多越前面，再依姓名排序；推薦卡片改為同時顯示所有符合的標記（例：`同科目／同任課班級／兼課`）。

**確保列出全部可代課教師**：候選名單原本只取「教師名單」，改為「教師名單 ∪ 課表上出現的所有教師」，只存在於課表、尚未加入名單者也會出現；姓名比對一律去除前後空白，避免因空白造成漏排除或重複。清單不截斷。

**驗證**：新增 `test/test-recommendation.mjs`（14 項，已加入 `npm test`），`npm test` 與 `npm run check` 全數通過；以 Playwright 在瀏覽器跑 V1 流程（`test/test-data.csv`，王大明 週一第一節 7年1班 數學）：10 位教師扣除原任課與該節有課者共列出 8 位，順序與標記符合預期，勾選兼課的同科目教師排在最後。**V2 教師管理表的兼課欄未在登入狀態下實測**（需 emulator 登入），僅通過語法檢查。

**重新匯入課表保留兼課設定**：課表檔不含兼課資訊，匯入時依姓名從匯入前的教師資料帶回 `partTime`（已在瀏覽器實測：勾選→重新匯入→仍勾選，localStorage 同步保留）。注意「導師班級」維持既有行為，重新匯入仍會被清空。
